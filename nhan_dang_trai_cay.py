...
def postprocess(frame, outs):
    ...
    if lastLayer.type == 'Region' or postprocessing == 'yolov8':
        ...
        if postprocessing == 'yolov8':
            box_scale_w = frameWidth / my_width
            box_scale_h = frameHeight / my_height
        else:
...
if img_file_buffer is not None:
    ...
    if st.button('Predict'):
        if not frame is None:
            ...
            inpWidth = my_width if my_width else frameWidth
            inpHeight = my_height if my_height else frameHeight
            blob = cv2.dnn.blobFromImage(frame, size=(inpWidth, inpHeight), swapRB=True, ddepth=cv2.CV_8U)

            # Run a model
            st.session_state["Net"].setInput(blob, scalefactor=0.00392, mean=mean)
